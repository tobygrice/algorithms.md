...
- $O(n+k)$: radix, counting, bucket (not comparison-based)
- $O(n \log n)$: quick, merge, heap
- $O(n^2)$: bubble, insertion, selection

**Quick Sort**
- *Divide and conquer partitioning around a pivot.*
- Time (best/avg/worst): $O(n \log n)$, $O(n \log n)$, $O(n^2)$
- Best/Worst case: Best/avg when pivots split ~even; worst when always min/max. Avoid via random pivot or shuffling input array.
- Stable? No. Cache-friendly? Moderate (sequential partitions). D&C? Yes. Comparison? Yes. In-place? Yes.
**Bubble Sort**
- What is it? Repeatedly swap adjacent out-of-order items.
- Time (best/avg/worst): $O(n)$, $O(n^2)$, $O(n^2)$
- Best/Worst case: Best when already sorted; worst when reverse sorted. Can’t fully avoid worst—just don’t use bubble.
- Stable? Yes. Cache-friendly? Yes. D&C? No. Comparison-based? Yes. In-place? Yes.
**Insertion Sort**
- What is it? Build sorted prefix by inserting next item into place.
- Time (best/avg/worst): $O(n)$, $O(n^2)$, $O(n^2)$
- Best/Worst case: Best when nearly sorted; worst when reverse. Avoid by using only on small/nearly-sorted data (e.g., hybrid sorts).
- Stable? Yes. Cache-friendly? Yes. D&C? No. Comparison-based? Yes. In-place? Yes.
**Merge Sort**
- What is it? D&C recursively split, merge sorted halves.
- Time (best/avg/worst): $O(n \log n)$, $O(n \log n)$, $O(n \log n)$
- Best/Worst case: Same across cases; worst is unavoidable but bounded at $O(n \log n)$.
- Stable? Yes. Cache-friendly? Moderate. D&C? Yes. Comparison-based? Yes. In-place? No (needs merge buffer).
**Selection Sort**
- What is it? Repeatedly select min from unsorted region, swap to front.
- Time (best/avg/worst): $O(n^2)$, $O(n^2)$, $O(n^2)$
- Best/Worst case: Always scans for min; no best improvement.
- Stable? No (swaps break stability). Cache-friendly? Yes. D&C? No. Comparison-based? Yes. In-place? Yes.
**Heap Sort**
- What is it? Build heap, repeatedly extract max to sorted tail.
- Time (best/avg/worst): $O(n)$, $O(n \log n)$, $O(n \log n)$
- Best/Worst case: Heap ops always log n; can’t avoid but bounded.
- Stable? No. Cache-friendly? Poor–moderate (random access). D&C? No. Comparison-based? Yes. In-place? Yes.
**Quick Select:**
- *A partial sorting algorithm based on quicksort, used to find the $k$th largest element in an unsorted array*
- Time (best/avg/worst): $O(n)$, $O(n)$, $O(n^2)$
...
- Worst-case lower bound: Ω(n log n) the minimum worst-case time for any comparison-based sort.

**Bucket Sort**
- What is it? Distribute into value-range buckets, sort buckets, concat.
- Time: $O(n + m)$ if uniform, else bucket-cost dominates.
- Best/Worst case: Best when uniform distribution; worst when all items fall into one bucket. Avoid via good bucket sizing or fallback sort per bucket.
- Stable? Depends on bucket sort; usually made stable. Comparison-based? No (not inherently). In-place? No.
**Counting Sort** 
- O(n + k) || O(n + k)
...
Singly-Linked List
- Use over an array/vector when you need regular insertions/deletions (especially if highly unknown size) and don't need random access
Doubly-Linked List
- Double-ended queue, push/pop at both ends in $O(1)$.
- Constant-time insertion before/after given node
...
- Consider vertex $u$. There are 2 units of flow at $u$, and the edges out of $u$ show where we can drain back those two units. 
	- The amount of flow at $u$ is the sum of edge weights from $u$ to the starting node (in the residual graph).
- Another example, consider $t$, the sink node. 
	- The residual flow tells us we can push 2 units out of $t$ back to $v$.
...
- The existence of a path from $s$ to $t$ indicates where we can push more flow.
- This lets us develop a maximum flow solution: ![[Pasted image 20260101212855.png]]
**Min cut**
- Has to be an $s$-$t$ cut ($s$ and $t$ are on different sides of the cut)
	- the partition containing $s$ can be called $A$, and the $t$ partition called $B$
- Draw an $s$-$t$ cut and sum the outward edges from $A$ to $B$ that cross the cut
- For all possible $s$-$t$ cuts, the one with the smallest sum is the minimum cut
- This sum of this minimum cut is equal to the maximum flow.
...
- Time:
	- $f$ is the maximum flow, worst case is you only increase $f$ by 1 in each iteration
...
- Advantage over Ford-Fulkerson: time doesn't depend on the capacities of the graph / the max flow value. Use if capacities can have many values.
...
## Other Topics
**Master Theorem (Time Complexity)**
- A shortcut for solving divide-and-conquer recurrences of the form:
	- `T(n) = a T(n/b) + f(n)`
**Dynamic Programming**
- *A method to solve problems by breaking them into overlapping subproblems and storing results (memoization/tabulation) to avoid recomputation, typically optimising for a recurrence.*
- Two approaches:
	- Memoization: **Top down**
		- Can be recursive but is generally implemented iteratively
		- Subproblems are only solved when they are reached
		- *Can* use a table, but it fills it lazily: entries are created only when a subproblem is actually visited during recursion.
		- e.g. compute the $n$th fibonacci number, starting with $F_0$ and $F_1$ and repeatedly caching $F_{i-1}$ and $F_{i-2}$
	- Tabulation: **Bottom-up**
		- Start at the bottom and move up to solution (computes all subproblems)
		- Iterative, fill a table in dependency order.
		- Entries are filled in a bottom-up manner from the smallest size to the final size.
		- e.g. compute **all** fibonacci numbers up to $n$ and store in table, use previously computed values to compute next value
- When to use:
	- Memoization/top-down when the state space is large but only a small portion is needed, or the recurrence is easier to express recursively.
	- Tabulation/bottom-up when you need to guarantee visiting all states once, want predictable iteration order, or better cache behaviour.

**P / NP / NP-hard / NP-complete:**
- **P:** the set of problems solvable in polynomial time
- **NP:** the set of problems verifiable in polynomial time (contains all of P)
- **NP-hard:** problems at least as hard as the hardest problems in NP. Everything reduces to them. Not necessarily in NP (may not be verifiable in polynomial time).
- **NP-complete:** in NP and NP-Hard (verifiable in polynomial time, everything reduces to them)
- Reductions:
	- All problems in NP can be reduced to an NP-Hard problem.
	- A reduction transforms an instance of problem **A** into an instance of problem **B**.
	- This shows that A is as hard as B.
	- Reduction must be:
		1. **Correct (solution-preserving)**: yes maps to yes, no maps to no
		2. **Deterministic**: transformation cannot rely on randomness or guessing.
		3. **Polynomial-time computable**: transformation from A → B must run in **O(n^k)** time.
		4. **No super-polynomial blow-up in size**: output instance size must be at most polynomial in the input size.
- Some NP-Complete problems and their reductions: ![[Pasted image 20251223114652.png]]
